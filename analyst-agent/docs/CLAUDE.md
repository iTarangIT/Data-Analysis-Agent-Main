...
**analyst-agent** — `app/api/` (HTTP only) · `app/services/` (business rules, domain errors) · `app/agent/` (LangGraph: `state.py`, `graph.py`, `prompts.py`, `nodes/`) · `app/connectors/` (customer sources: `base.py` protocol, `postgres.py`, `registry.py`) · `app/security/` (`auth.py` JWT, `vault.py` Fernet) · `app/db/` (App DB session + models: Tenant, Connection, Run, Usage) · `app/workers/` · plus `evals/`, `scripts/`, `tests/{unit,integration}`. Nothing imports upward. `HTTPException` is raised only inside `app/api/`; everything else raises from `app/services/errors.py`.

- `AgentState` is a `TypedDict(total=False)` so nodes return partial updates and LangGraph merges them. `tool` is `Literal["sql", "web", "clarify"]`; `clarify` routes straight to `answer`.
- `app/agent/nodes/sql_guard.py` is the most important file in the service and is **pure sqlglot code that must never call a model**: SELECT only, one statement, table allowlist from the connection's schema cache, LIMIT injected, and every mutating expression type rejected (`Insert`, `Update`, `Delete`, `Drop`, `Alter`, `Create`, `Command`, `Merge`, `TruncateTable`, `Grant`).
